/* =====================================================
   The album — photos revealed when the ridgeline lifts.
   -----------------------------------------------------
   To use real troop photos:
     1. Drop image files into the /photos folder.
     2. Add entries below — just the filename for local files,
        or a full URL. Captions are optional.

   Example:
     { src: 'philmont-2025.jpg', caption: 'Philmont, 2025' }

   The photo wall shows 12 tiles (4 x 3) and cycles through
   this list if there are fewer than 12. Square-ish images
   look best. The placeholders below are stock images.
   ===================================================== */

window.GALLERY = [
    { src: 'https://picsum.photos/seed/troopalbum1/800/800', caption: '[ Caption ]' },
    { src: 'https://picsum.photos/seed/troopalbum2/800/800', caption: '[ Caption ]' },
    { src: 'https://picsum.photos/seed/troopalbum3/800/800', caption: '[ Caption ]' },
    { src: 'https://picsum.photos/seed/troopalbum4/800/800', caption: '[ Caption ]' },
    { src: 'https://picsum.photos/seed/troopalbum5/800/800', caption: '[ Caption ]' }
];
